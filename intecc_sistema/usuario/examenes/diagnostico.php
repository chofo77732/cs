...
$sql_ya="SELECT * FROM `historial` WHERE id_usuario = '$id_user' and tipo = \"diagnostico\" ";
...
$sql_reactivos="SELECT * FROM reactivos WHERE nivel = ( SELECT MAX( nivel )  FROM reactivos WHERE id_tema='$id_t') and id_tema='$id_t' and pregunta!=\"\" and correcto=\"1\" ORDER BY rand() LIMIT 1";
...
$orden=array();
$i=0;
      $sql_r="SELECT DISTINCT * FROM reactivos WHERE id_reactivo in ($cad) ORDER BY rand() limit 10";
...
for (var i = 0 ; i < orden.length; i++) {
  ...
  if(valor==6){
$('#rok'+orden[i]).html('<h3>bien'+valor+'</h3>');

  $.ajax({
      url:'respuesta.php',
      type:'POST',
      data:'user='+user+'&reactivo='+orden[i]+'&curso='+curso+'&tema='+tema+'&respuesta=1&tipo=diagnostico&boton=insertar'
  }).done(function(resp){
      if(resp=='error'){
          // alert(resp);
      }else{
    ...
    $('#rok'+orden[i]).html('<h3>mal'+valor+'</h3>');

      $.ajax({
      url:'respuesta.php',
      type:'POST',
      data:'user='+user+'&reactivo='+orden[i]+'&curso='+curso+'&tema='+tema+'&respuesta=0&tipo=diagnostico&boton=insertar'
  }).done(function(resp){
      if(resp=='error'){
          // alert(resp);
      }else{
